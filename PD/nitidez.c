...
// Que indica o custo de cada impressão (para debugar)
...
int imprimeNitido (char* palavra[], int* l, int* sp, int n, int M) {
    ...
    // Para cada palavra k, calcula o custo de colocá-la em cada linha li
    for (int k = 1; k < n; k++) {
...
int main (int argc, char* argv[]) {
    ...
    for (int i = 0; i < n; i++) {
        l[i] = 0;
        palavra[i] = malloc (30 * sizeof( char));
        // Le a palavra e guarda o tamanho delas
        char c = getchar() ;
        while (c == ' ' || c == '\n') c = getchar();
        while (c != ' ' && c != '\n' && c != '\0') {
            palavra[i][l[i]++] = c; 
